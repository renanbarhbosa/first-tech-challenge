...
output_dir = r"C:\intellij-projects\postech\first-tech-challenge\spain-micro-analysis"
...
def convert_columns(df):
    ...
    return df.withColumn(
        ...
        (F.lower(F.col("País")).contains("espanha")) &
        (F.col("Quantidade") > 0) &
        (F.col("Valor") > 0)
    )
...
if __name__ == "__main__":
    spark = SparkSession.builder.appName("SpainOtherGrapeProductsExports").getOrCreate()
    spark.sparkContext.setLogLevel("WARN")

    try:
        products = ["fresh_grapes", "grape_juice"]
        all_data = []
        ...
        save_plot(pd_df, 'Year', 'Total_Valor',
                  'Evolução do Valor das Exportações de uvas e derivados para Espanha',
                  'uvas_derivados/exportacoes_uva_derivados_valor.png', 'line')

        save_plot(pd_df, 'Year', 'Total_Quantidade',
                  'Evolução da Quantidade Exportada de uvas e derivados para Espanha',
                  'uvas_derivados/exportacoes_uva_derivados_quantidade.png', 'bar')
